...
public:
	...
	iterator();
	~iterator();
      iterator(vector<Point> vector, int index);
	...
	//
      ...
      //QTQuad* m_current;
      //BBox m_current_bounds;
      //      Point m_pt;
      //static Point getEndPoint(const BBox& bounds);
      //static Point getNextPoint(const Point &pt, const QTQuad *current, const BBox &bounds);
      //static void treverseTree(const BBox &bounds);
      vector<Point> m_vectorInIterator;
      int m_index;
