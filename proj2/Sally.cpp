  ...
  istrm(input_stream)  // use member initializer to bind reference
{
  ...
  symtab["!"]    =  SymTabEntry(KEYWORD,0,&doStoreValue);
  
  symtab["<"]    =  SymTabEntry(KEYWORD,0,&doLess);
  symtab["<="]   =  SymTabEntry(KEYWORD,0,&doLessAndEqu);
  symtab["=="]   =  SymTabEntry(KEYWORD,0,&doEqual);
  symtab["!="]   =  SymTabEntry(KEYWORD,0,&doNotEqual);
  symtab[">="]   =  SymTabEntry(KEYWORD,0,&doGreaterAndEqu);
...
void Sally::doDup(Sally *Sptr){
  ...
  Sptr->params.push( Token(INTEGER, p.m_value, p.m_text) ) ;
}
...
void Sally::doSet(Sally *Sptr){
  ...
  if(Sptr->symtab.find(p1.m_text) != Sptr->symtab.end()) {
    cerr << "keyword alreay defined before" << endl;
  }else{
    Sptr->symtab[p1.m_text] = SymTabEntry(INTEGER,p2.m_value,NULL);
  }
}
...
void Sally::doAt(Sally *Sptr){
  ...
  Sptr->params.pop() ;
  if(Sptr->symtab.find(p1.m_text) == Sptr->symtab.end()){
    cerr << "keyword dosen't exist " << endl;
  }else{
    SymTabEntry temp = Sptr->symtab[p1.m_text];
    Sptr->params.push(Token(INTEGER, temp.m_value,""));
  }
}

void Sally::doStoreValue(Sally *Sptr){
  Token p1, p2;
  if(Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for !");
  }
  p1 = Sptr->params.top(); //variable name
  Sptr->params.pop();
  p2 = Sptr->params.top();
  Sptr->params.pop();
...
void Sally::doLessAndEqu(Sally *Sptr){
  Token p1, p2;
  if(Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for <=.");
  }
...
void Sally::doGreaterAndEqu(Sally *Sptr){
  Token p1, p2;
  if(Sptr->params.size() < 2){
		throw out_of_range("Need two parameters for >=.");
  }
...
void Sally::doNot(Sally *Sptr){
  Token p;
  if(Sptr->params.size() < 1){
    throw out_of_range("Need two parameters for NOT");
  }
  p = Sptr->params.top();
  Sptr->params.pop();
...
void Sally::doDo(Sally *Sptr){
  ...
  for(list<Token>::iterator it = Sptr->tempBuffer.begin(); it != Sptr->tempBuffer.end(); ++it){
